...
const Edit = ( props ) => {
	...
	const { attributes, setAttributes, clientId } = props,
		wrapArgs = attributes?.ID && mergeWrapperProps( { 
			className: `wpmozo-background-switcher${ attributes?.wrapIsHover ? ' is_hover' : '' }` ,
			style: {}
		}, attributes ),
	...
	const addChildBlock = () => {
		const newBlock = createBlock( 'wpmozo/background-switcher-item', { text: childAttributes.text } );
					...
					<div className="wpmozo_bna_background_switcher_inner">
						...
						<InnerBlocks
							...
							template={ [
								...
								[ 'wpmozo/background-switcher-item', { itemBackground: '#f1f1f1' } ],
								[ 'wpmozo/background-switcher-item', { itemBackground: '#cccccc' } ],
								[ 'wpmozo/background-switcher-item', { itemBackground: '#e0e0e0' } ],
							...
							renderAppender={ () => (
								<button type="button"
									...
									title={ __( 'Add New Item', 'wpmozo-blocks-and-addons' ) } // Tooltip text.
								>
