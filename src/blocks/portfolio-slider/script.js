...
$( document ).ready( function(e) {
	...
	window.addEventListener( 'WPMozoPortfolioPropsChanged', () => {
		$( '.wp-block-wpmozo-portfolio-slider' ).each( function() {
			initWPMozoPortfolioSlider( $( this ) );
		} );
	} );
	...
	if ( $( document ).find( '.wp-block-wpmozo-portfolio-slider' ).length > 0 ) {
		$( document ).find( '.wp-block-wpmozo-portfolio-slider' ).each( function() {
...
function initWPMozoPortfolioSlider( blockObj ) {
	...
	if ( 'slide' === slideEffect || 'coverflow' === slideEffect ) {
		...
		$slidesPerGroupMobile		  = ( settings?.per_group_mobile ) ? settings.per_group_mobile : $slidesPerGroupIpad;
