...
function WPMozoPriceListLayout({ props }) {
  ...
  const title = (
    <RichText
      className="wpmozo-bna-image-accordion-item-name"
      tagName={HeadingTag}
  ...
  if (itemPrice) {
    price = <span className="wpmozo-bna-image-accordion-item-price">{itemPrice}</span>;
  }

  if (itemCurrency) {
    currency = (
      <span className="wpmozo-bna-image-accordion-item-currency">{itemCurrency}</span>
    );
  }

  description = (
    <RichText
      className="wpmozo-bna-image-accordion-item-description"
      tagName="div"
      value={ attributes.content }
      onChange={ ( newValue ) => setAttributes( { content: newValue } ) } 
      placeholder={ __('Item Description', 'wpmozo-blocks-and-addons') }
    />
  );
...
function layout1(
  ...
  if (thumbnail && itemThumbnailOption === "use_image") {
    innerWrap.push(
      <div
        className="wpmozo-bna-image-accordion-item-thumbnail"
  ...
  if (thumbnail && itemThumbnailOption === "use_icon") {
    ...
    innerWrap.push(
      <div
        className="wpmozo-bna-image-accordion-item-icon"
  ...
  if (price) {
    itemPriceWrap.push(
      <div
        className="wpmozo-bna-image-accordion-item-price-divider"
        key={`${address}_layout1_price_divider`}
      />
    );
    itemPriceWrap.push(
      <div
        className="wpmozo-bna-image-accordion-item-price-wrap"
    ...
    if (pricePeriod) {
      itemPriceWrap.push(
        <div
          className="wpmozo-bna-image-accordion-item-price-period"
  ...
  itemNameWrap.push(
    <div
      className="wpmozo-bna-image-accordion-item-name-wrap"
  ...
  innerWrap.push(
    <div
      className="wpmozo-bna-image-accordion-item-details"
  ...
  return (
    <div className="wpmozo-bna-price-list-layout wpmozo-bna-price-list-layout1" key={`${address}_layout1_item`}>
      <div className="wpmozo-bna-image-accordion-item-wrap">{innerWrap}</div>
    </div>
  );
}
...
function layout2(
  ...
  if (price) {
    ...
    if (pricePeriod) {
      pricePeriodHtml = (
        <span className="wpmozo-bna-image-accordion-item-price-period">
          <span className="wpmozo-bna-price-period-divider">/</span>
          {pricePeriod}
        </span>
      );
