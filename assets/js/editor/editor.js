...
jQuery(document).ready(function(){
	...
	jQuery(document).on("click", "body", function(e) {
		...
		if ( target.closest('.wpmozo-bna-logo-slider-wrap').length < 1 ) {
			return;
		}
		...
		if ( target.closest('.swiper-slide').length > 0 ) {
			...
			jQuery(".swiper-slide.block-editor-block-list__block").closest('.wpmozo-bna-logo-slider-main').removeClass('has-child-selected');
			jQuery(".swiper-slide.block-editor-block-list__block").removeClass(selectedClass);

			if ( target.closest('.wpmozo-bna-logo-slider-main').length > 0 ) {

				let slider = target.closest('.wpmozo-bna-logo-slider-main'),
